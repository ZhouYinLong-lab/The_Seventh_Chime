...
export type PlaytestEventKind = 'query' | 'invalid_query' | 'unlock' | 'revisit' | 'compare' | 'hint' | 'b4_reveal' | 'hypothesis_edit' | 'ring_submit' | 'modified_frame_reveal' | 'modified_frame_edit' | 'modified_frame_submit';
export interface PlaytestEvent { kind: PlaytestEventKind; at: string; detail?: Record<string, string | number | boolean> }
export interface SaveV2 { version: 2 | 3; discovered: string[]; read: string[]; annotations: Record<string, string[]>; notes: Note[]; hypotheses: HypothesisGrid; queryHistory: QueryHistoryEntry[]; pinnedDocIds: string[]; compareDocIds: string[]; archiveFilters: ArchiveFilters; stageSubmissions: StageSubmissions; draftOriginalRing: BodyId[]; hintState: HintState; playtestEvents: PlaytestEvent[]; activeDoc: string | null; activeSegmentId: string | null; query: QueryState; attempts: number; tab: 'query' | 'archive' | 'facts' | 'notes' | 'hypotheses'; updatedAt: string }
