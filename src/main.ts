...
// Esc 监听挂 document：innerHTML 全量重渲染会销毁聚焦元素，焦点回落到 body，app 级监听收不到冒泡事件。卷首优先于档案浮层。
document.addEventListener('keydown', (event) => { if (event.key !== 'Escape') return; if (introOpen) { event.preventDefault(); closeIntro(); } else if (overlayOpen) { event.preventDefault(); closeFile(); } });
