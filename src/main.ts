...
const importSave = (file: File) => file.text().then((text) => { try { const parsed = JSON.parse(text) as { save?: unknown }; const imported = migrateSave(parsed.save ?? parsed, content.characters, content.documents); if (!imported) throw new Error('bad save'); state = imported; overlayOpen = false; activeArchive = null; feedback = '进度已导入；旧版存档已安全迁移到当前结构。'; saveAndRender(); } catch { feedback = '导入失败：文件包含损坏或不存在的档案、推演或引用，现有进度未被覆盖。'; render(app, state, feedback, isB4Revealed(state.discovered), storageNotice, { activeArchive, sealNotice, overlayOpen, drawers }); } });
...
const compareByKeys = (left: string, right: string): string[] => {
  ...
  state.compareDocIds = [first.doc.id, second.doc.id]; overlayOpen = true;
...
// Esc 监听挂 document：innerHTML 全量重渲染会销毁聚焦元素，焦点回落到 body，app 级监听收不到冒泡事件。
document.addEventListener('keydown', (event) => { if (event.key === 'Escape' && overlayOpen) { event.preventDefault(); closeFile(); } });
...
app.addEventListener('click', (event) => {
  ...
  if (action === 'open-archive') { const archiveId = button.dataset.archive || ''; if (archiveById.has(archiveId)) { activeArchive = archiveId; state.activeDoc = null; state.activeSegmentId = null; sealNotice = null; overlayOpen = true; shouldPosition = false; state.tab = 'archive'; } return saveAndRender(); }
...
render(app, state, feedback, isB4Revealed(state.discovered), storageNotice, { activeArchive, sealNotice, overlayOpen, drawers });
